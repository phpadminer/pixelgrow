...
assert(
  wxml.includes('bindchange="onMyRelationChange"')
    ...
    && wxml.includes('{{myRelationLabel}}')
    && !wxml.includes('relationOptions[myRelationIndex].label')
    && wxml.includes('data-member-id="{{item.id}}"')
    && wxml.includes('wx:if="{{item.canManageRelation}}"')
    && wxml.includes('wx:if="{{item.canManageRole}}"')
    && /myRelationLabel: '暂不设置'/.test(pageJs)
    && /myRelationLabel: FAMILY_RELATION_OPTIONS\[myRelationIndex\]\.label/.test(pageJs)
