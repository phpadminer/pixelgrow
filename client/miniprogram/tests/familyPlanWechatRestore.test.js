...
assert(
  /isWechatRestoreStillCurrent\(\)[\s\S]*?!this\.data\.guestModeStarted[\s\S]*?wx\.getStorageSync\(GUEST_SESSION_KEY\)/.test(pageJs)
    && /async restoreWechatSessionOnStart\(\)[\s\S]*?if \(!this\.isWechatRestoreStillCurrent\(\)\) return[\s\S]*?this\.chooseGuestMode\(\)/.test(pageJs)
    && /chooseGuestMode\(\)[\s\S]*?guestModeStarted: true/.test(pageJs),
  'late silent restore results should not reopen login choices and should fall back to guest browsing'
)
